...
## AI-адаптивная маскировка трафика (экспериментально)

В [`core/`](core/) добавлен самодостаточный Go-модуль `aiobfs` — слой
адаптивной маскировки трафика, который постоянно меняет его "вид"
(размеры пакетов, тайминги, полезную нагрузку RTP-заголовка), выбирая
между несколькими правдоподобными профилями (аудио-звонок, видео-звонок,
демонстрация экрана, keepalive) с помощью лёгкой онлайн-обучаемой модели —
адверсариального бандита (EXP3) и маленькой нейросети (5→8→N), а не
большой LLM в пути пакета (это было бы слишком медленно и свело бы на нет
цель "ускорить интернет"). Он реагирует на реальные измерения RTT/потерь и
сам уводит трафик от той маскировки, которую, судя по всему, научились
блокировать/душить — не дожидаясь ручной перенастройки.

Это не замена существующему TURN/DTLS/VK-звонки движку (тому самому
`turn-proxy`, о котором говорится выше) — реализация VK-credentials,
STUN/TURN и DTLS-рукопожатия уже решена в апстриме и в форках сообщества,
и дублировать её здесь бессмысленно и рискованно. `aiobfs.Shaper`
спроектирован как готовый к встраиванию заменитель статичного слоя
обфускации (`obfs.go`-подобного) в таком движке — см.
[`core/README.md`](core/README.md) для архитектуры, гарантий безопасности,
инструкции по интеграции и живой демонстрации:

```sh
cd core
go test ./...
go run ./cmd/aiobfs-demo
```
